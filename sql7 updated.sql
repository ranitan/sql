...
insert into manager values(101,'ranita')
...
delete from manager where m_id=100
...
update manager set  where m_id=103

--combination

alter table emp drop constraint [FK__emp__m_id__6E8B6712]
alter table emp drop column m_id
alter table emp add m_id int default 101 constraint fk_emp foreign key(m_id) references manager(m_id) on delete cascade on update set default

update emp set m_id=101 where e_id=1
update emp set m_id=102 where e_id=2
update emp set m_id=103 where e_id=3
update emp set m_id=104 where e_id=4

delete from manager where m_id=101

update manager set m_id=106 where m_id=102